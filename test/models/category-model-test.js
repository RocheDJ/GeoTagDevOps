...
import {testDBType, testCategories, Kiln, testPOI, demoUser, demoCategory} from "../fixtures.js";
...
suite("Category Model tests", () => {
  ...
  test("Get User Categories", async () => {
    ...
    // add categories for tha user
    const demoCat = demoCategory;
    demoCat.userid = newUser._id;
    const newCat = await db.categoryStore.addCategory(demoCat);
    const userCat = await db.categoryStore.getUserCategories(newUser._id);
    assert.equal(newCat._id,userCat[0]._id);
  });
